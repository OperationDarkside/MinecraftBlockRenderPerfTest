		...
		for (int x = 0; x < CHUNK_SIZE; ++x) {
			for (int y = 0; y < CHUNK_SIZE; ++y) {
				for (int z = 0; z < CHUNK_SIZE; ++z) {
					...
					float xPos = x;
					float yPos = y + model.y;
					float zPos = z;

					// FRONT
					if (renderData.front) {
						std::array<float, 20> vboFrontData {
							xPos, yPos, zPos + 1, type.front[0], type.front[1],
							xPos + 1, yPos, zPos + 1, type.front[2], type.front[3],
							xPos, yPos + 1, zPos + 1, type.front[4], type.front[5],
							xPos + 1, yPos + 1, zPos + 1, type.front[6], type.front[7]
						};
					...
					if(renderData.back){
						std::array<float, 20> vboBackData {
							xPos + 1, yPos, zPos, type.back[0], type.back[1],
							xPos, yPos, zPos, type.back[2], type.back[3],
							xPos + 1, yPos + 1, zPos, type.back[4], type.back[5],
							xPos, yPos + 1, zPos, type.back[6], type.back[7]
						};
					...
					if(renderData.right){
						std::array<float, 20> vboRightData {
							xPos + 1, yPos, zPos + 1, type.right[0], type.right[1],
							xPos + 1, yPos, zPos, type.right[2], type.right[3],
							xPos + 1, yPos + 1, zPos + 1, type.right[4], type.right[5],
							xPos + 1, yPos + 1, zPos, type.right[6], type.right[7]
						};
					...
					if(renderData.left){
						std::array<float, 20> vboLeftData {
							xPos, yPos, zPos, type.left[0], type.left[1],
							xPos, yPos, zPos + 1, type.left[2], type.left[3],
							xPos, yPos + 1, zPos, type.left[4], type.left[5],
							xPos, yPos + 1, zPos + 1, type.left[6], type.left[7]
						};
					...
					if(renderData.top){
						std::array<float, 20> vboTopData {
							xPos, yPos + 1, zPos + 1, type.top[0], type.top[1],
							xPos + 1, yPos + 1, zPos + 1, type.top[2], type.top[3],
							xPos, yPos + 1, zPos, type.top[4], type.top[5],
							xPos + 1, yPos + 1, zPos, type.top[6], type.top[7]
						};
					...
					if(renderData.bottom){
						std::array<float, 20> vboBottomData {
							xPos, yPos, zPos, type.bottom[0], type.bottom[1],
							xPos + 1, yPos, zPos, type.bottom[2], type.bottom[3],
							xPos, yPos, zPos + 1, type.bottom[4], type.bottom[5],
							xPos + 1, yPos, zPos + 1, type.bottom[6], type.bottom[7]
						};
